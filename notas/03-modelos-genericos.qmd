...
**Modelo estadístico**: Nuestro diagrama causal justifica, aún cuando no tengamos la variable de ángulo, el uso de un modelo genérico como regresión logística (por ejemplo,
no es necesario controlar por velocidad del tiro, y tampoco no nos interesa saber la distribución
de distancias a la que intentan un putt), donde
usamos $Y$ como respuesta y $D$ como variable independiente. Planteamos entonces un 
modelo logístico:
...
Valores imposibles para golfistas profesionales, por ejemplo:
...
grafs_tbl <- sims |>
  rowwise() |>
  mutate(graf = list(tibble(d = 30.48 * datos_golf$x) |> 
           mutate(p = 1 / (1 + exp(-alpha - beta * d)))
  )) |> 
  ungroup() |> 
  slice_sample(n = 100) |> 
...
![Tiro de golf](figuras/golf-put.png)
Tenemos que
...
que un tirador profesional tira con un ángula $\theta$ que se distribuye
como
...
map_df(1:100,  \(x) simular_putts_angulo(distancias) |> mutate(id = x)) |> 
  ...
  labs(x = "Distancia (cm)", y = "Probabilidad de Éxito")
```
Podemos comenzar con esta inicial, que espera que en general
...
ajuste_2 <- mod_golf$sample(
  data = list(N = nrow(datos_golf),
              d =  30.48 * datos_golf$x, y = datos_golf$y, n = datos_golf$n),
  refresh = 1000, init = 0, step_size = 0.01)
sims_2 <- ajuste_2$draws(c("sigma"), format = "df")
resumen_2 <- ajuste_2$summary("sigma")
