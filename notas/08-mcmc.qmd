...
el origen del vino (que tiene dos niveles):
...
calificar de los jueces. En primer lugar, definimos un nivel general $H$ que indica qué
...
$$\mu_{i} = Q_{vino(i)} + U_{origen(i)} - H_{juez(i)}$$
...
$$\mu_{i} = (Q_{vino(i)} + U_{origen(i)} - H_{juez(i)})D_{juez(i)}$$
